  ...
  (
    ...
    const hide = useCallback(() => {
      Animated.timing(opacityAnimation.current, {
        ...
        menuSizeAnimation.current.setValue({x: 0, y: 0}); // = new Animated.ValueXY({x: 0, y: 0});
        opacityAnimation.current.setValue(0); // = new Animated.Value(0);
      });
    }, [animationDuration]);
    const menuSize = {
    ...
    const animationStarted = useMemo(() => menuState === STATES.ANIMATING, [
      menuState,
    ]);
